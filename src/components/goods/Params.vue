  ...
  methods: {
    ...
    getParamsData() {
      ...
      if (this.selectedCateKeys.length !== 3) {
        ...
        paramsManageCateListData(this.cateId, this.activeName).then((res) => {
          if (res.meta.status !== 200) {
    ...
    showEditDialog(attr_id) {
      // 查询当前参数的信息
      queryParamsData(this.cateId, attr_id, this.activeName).then((res) => {
        if (res.meta.status !== 200) {
    ...
    removeParams(attr_id) {
      this.$confirm("此操作将永久删除该参数, 是否继续?", "提示", {
        ...
        .then(() => {
          deleteParamsData(this.cateId, attr_id).then((res) => {
            if (res.meta.status !== 200) {
